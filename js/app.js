...
(function(){
  ...
  /** 
   * A stab at basic responsive behavior for the map container. 
   * Does not rebuild or adjust the map inself.
   */
  ...
  function resize() {
    ...
    document.getElementById('map').style.height = height + 'px';
    d3.select("#map svg").attr("width", width).attr("height", height);
    d3.select(".viewport").attr("width", width).attr("height", height);
  }
...
window.onload = passportMap.init;
window.onresize = passportMap.resize;